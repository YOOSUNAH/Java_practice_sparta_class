package task.task5_sortingBooks;
...
public class Main {
    public static void main(String[] args) {
        ...
        // 카테고리가 여행인 책 제목 조회
        bookList.stream().filter(book -> book.getCategory()
                .equals("여행"))
            .forEach(p -> System.out.println("카테고리가 여행인 책 제목 : " + p.getBookName()));
        System.out.println();

        // 가격이 16200원 이하인 책 제목 조회
        bookList.stream().filter(book -> book.getPrice() <= 16200)
            .forEach(p -> System.out.println("가격 16,200원 이하의 책 제목 : " + p.getBookName()));
        System.out.println();

        // 책 제목에 "경제" 라는 용어가 들어간 책 제목 조회
        bookList.stream().filter(book -> book.getBookName().contains("경제"))
            .forEach(p -> System.out.println("책 제목에 \"경제\" 라는 용어가 들어간 책 제목 : " + p.getBookName()));
        System.out.println();

        // 가격이 가장 비싼 책 가격 조회
        // mapToDouble() : 스트림을 double로 변환
